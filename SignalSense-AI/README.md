...
**SignalSense AI** is an advanced web application designed to solve cellular coverage opacity, dead-zone estimation, and carrier benchmarking. By combining **real-time OpenStreetMap Overpass mobile antenna telemetry**, **Inverse Distance Weighting (IDW) spatial RF interpolation**, and **multi-source geocoding with alias resolution**, SignalSense AI allows users to map cell towers, benchmark carriers, and predict signal dead-zones for any location worldwide.

---

## 🔥 Key Features

- 🔐 **Futuristic Dual-Panel Glassmorphism Auth System**: Secure user authentication with 10-digit mobile verification, quick demo auto-fill, platform feature highlights, and ambient glowing backgrounds.
...
- 🔍 **Sub-Locality Geocoding & Institute Alias Engine**: High-precision location engine that accurately identifies educational institutes, landmarks, coaching hubs, and cities (*e.g., Madhav Institute of Technology & Science (MITS Gwalior), ABV-IIITM Gwalior, LNIPE, Gwalior Fort, Rajeev Gandhi Nagar, Landmark City, BKC Mumbai, Connaught Place Delhi*). Features automated acronym expansion and Photon API fallback.
...
- **External APIs**: 
  ...
  - Komoot Photon Geocoding Fallback API (`https://photon.komoot.io/api`)
...
3. **Start Development Server**:
   ...
   npm run dev
   ```
   This launches both the Express backend API server (`http://localhost:5000`) and the Vite React frontend application (`http://localhost:3000`).
...
| :--- | :--- | :--- |
| `POST` | `/api/auth/login` | Authenticates user with username & 10-digit mobile number |
| `GET` | `/api/measurements?lat={lat}&lng={lng}&carrier={carrier}` | Retrieves carrier cell tower node measurements around coordinates |
| `POST` | `/api/measurements` | Submits a new user-reported cellular telemetry measurement |
| `GET` | `/api/prediction?lat={lat}&lng={lng}&carrier={carrier}&nodeId={id}` | Calculates IDW spatial AI prediction & dead-zone risk |
| `GET` | `/api/carriers?lat={lat}&lng={lng}` | Computes carrier leaderboard benchmarks and rankings |
| `GET` | `/api/heatmap?lat={lat}&lng={lng}` | Generates intensity values for cellular signal heatmap overlays |
...
- **Futuristic Glassmorphic Auth Portal**: Split dual-panel login screen featuring ambient lighting, credential validation, platform highlights, and quick demo access.
